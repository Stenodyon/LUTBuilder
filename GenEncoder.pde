/*
   ...
   This algorithm uses the path argument to collect the source file, but it
   doesn't use the reference, facing, and side arguments to move and rotate the
   circuit.
   If you can figure out how to apply these arguments, as well as optimize this
   algorithm for speed, feel free to modify this all you like.
   The algorithm works just like before, calling the typeString function for
   print it's final results to the game,
   however all instances of println() have been replaced with 'splash = '
   This allows the splash to notify the user of the progress.
   I've also added the progress bar to go along side the splash. In this case,
   it's just showing how much each for loop has progressed, more details in the
   'ProgressBar' tab
   At this point it moves very rigidly, since it's updated once per iteration
   in the for loops, ideally I'd like it to progress smoothly, but that's not
   necessary.
 */
